/**
 * Static metadata for the DetailModal — the metric union, per-metric
 * labels + descriptions + optional CLI hints, the server-export
 * allow-list, and the event-trigger mapping.
 *
 * Extracted from DetailModal.tsx so the modal component stays focused
 * on UI logic; this file is pure data + types.
 */
...
export const EXPORTABLE: ReadonlySet<MetricKind> = new Set<MetricKind>([
...
export const META: Record<MetricKind, MetricMeta> = {
  trajectory_mean: {
...
export const INJECT_TRIGGERS: Partial<Record<MetricKind, InjectMapping>> = {
