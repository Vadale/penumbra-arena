...
Concept taught: how a trained model lands in production. The
`Simulation.build()` accepts a `policy_factory(agent_id) -> Policy`,
where `Policy` is `Callable[[AgentObservation, np.random.Generator],
NodeId]`. This module loads a checkpoint once and returns a factory
that hands every agent a `Policy` closure backed by the same MAPPO
actor — parameter sharing, exactly as the model was trained.

The factory is deliberately resilient: if the checkpoint can't be
loaded (missing file, wrong shape, MPS unavailable), we log and fall
back to the random-walk baseline so the simulation always boots.
"""
