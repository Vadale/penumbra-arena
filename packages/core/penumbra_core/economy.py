...
# Back-compat shim: old code still imports PurchaseClock. We keep the
# name but it now delegates to Market under the hood. Result format
# matches the old Purchase dataclass for any consumer that hasn't moved
# to Trade yet.
@dataclass(slots=True)
class PurchaseClock:
    """Deprecated: use Market.tick(). Kept as a back-compat shim."""

    inventories: dict[int, CityInventory]
    market: Market | None = None
    p_buy_per_item: float = _P_BUY
    avg_quantity: float = _AVG_BUY_QUANTITY
    ...
    def settle_tick(
        ...
        """Delegates to Market.tick() and converts the buys back to Purchases."""
        if self.market is None:
            return []
        trades = self.market.tick(tick, agent_positions, rng)
        return [
            Purchase(
                tick=t.tick,
                agent_id=t.agent_id,
